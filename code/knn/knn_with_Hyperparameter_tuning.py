...
def knn_grid_search():
    ...
    knn_best = grid_object.best_estimator_
    print(knn_best)
...
"""     # Visualize the grid search results
    ...
    for param, param_values in grid_params.items():
        param_name = f"param_{param}"

        # Plotting the mean test scores
        sns.lineplot(x=param_name, y='mean_test_score', data=results)
        
        # Highlighting the best value
        best_value = grid_object.best_params_[param]
        plt.scatter(best_value, knn_best.score(train_x, train_Y), marker='x', color='red', zorder=10)
...
KNeighborsClassifier(leaf_size=10, n_jobs=-1, n_neighbors=2) """